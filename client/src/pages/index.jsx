...
const getNotifIcon = (type) => {
  switch (type) {
    ...
    case 'task': return <FolderKanban className="w-5 h-5 text-purple-500" />;
    case 'proposal': return <Sparkles className="w-5 h-5 text-amber-500" />;
    case 'system': return <CheckCircle2 className="w-5 h-5 text-green-500" />;
    default: return <Bell className="w-5 h-5 text-surface-500" />;
  }
};

const getNotifLink = (n) => {
  if (n.referenceModel === 'Project') return `/projects/${n.referenceId}`;
  if (n.referenceModel === 'Task' && n.projectId) return `/projects/${n.projectId}/tasks`;
  if (n.referenceModel === 'Friendship') return `/chat/${n.referenceId}`;
  return '#';
};

export const NotificationsPage = () => {
  const { notifications, isLoading } = useNotifications();
  const { mutate: markAllRead, isLoading: isMarkingAll } = useMarkAllRead();
    ...
    <div className="max-w-4xl mx-auto h-full flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-sm font-semibold text-primary-600 hover:text-primary-700 hover:bg-primary-50 px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
        >
          Mark all as read
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-surface-200 shadow-sm flex-1 overflow-hidden flex flex-col">
        {isLoading ? (
          <div className="p-8 space-y-4">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-20 bg-surface-100 rounded-xl animate-pulse"></div>
            ))}
          </div>
        ) : notifications.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[400px]">
            <div className="w-16 h-16 bg-surface-50 rounded-full flex items-center justify-center mb-4 border border-surface-200 shadow-sm">
              <Bell className="w-8 h-8 text-surface-400" />
            </div>
            <h3 className="text-lg font-bold text-surface-900 mb-1">You're all caught up!</h3>
            <p className="text-surface-500">No new notifications right now.</p>
          </div>
        ) : (
            ...
            <div className="divide-y divide-surface-100">
              {notifications.map((n) => (
                <div key={n._id} className={cn(
                  "relative flex sm:items-center gap-4 p-5 hover:bg-surface-50 transition-colors group",
                  ...
                  <div className="w-12 h-12 rounded-full bg-white border border-surface-200 shadow-sm flex items-center justify-center shrink-0">
                    {getNotifIcon(n.type)}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <p className="text-sm font-semibold text-surface-900">
                        {n.title || (n.type === 'message' ? 'New Message' : n.type === 'task' ? 'Task Update' : 'Notification')}
                      </p>
                      <p className="text-sm text-surface-600 mt-0.5 leading-snug break-words">
                        {n.message}
                      </p>
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                      ...
                      {!n.read && (
                        <button
                          ...
                          className="p-1.5 text-surface-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                          title="Mark as read"
                        >
