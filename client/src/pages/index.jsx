...
export const NotificationsPage = () => {
  ...
  export const ProfilePage = () => {
    ...
    const inputCls = "block w-full px-3 py-2 border border-surface-200 rounded-lg text-sm text-surface-900 placeholder-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent";
    ...
    return (
        <div className="max-w-4xl mx-auto h-full flex flex-col gap-6 pb-12">
          ...
          {/* Edit Mode */}
          {isEditing ? (
            <div className="bg-white rounded-2xl border border-surface-200 p-8 shadow-sm space-y-6">
              <div>
              ...
              <div>
                ...
                <input type="text" className={inputCls} placeholder="e.g. Full-stack developer | Open source enthusiast"
              ...
              <div>
                ...
                <input type="text" className={inputCls} placeholder="e.g. San Francisco, CA"
                  value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-surface-700 mb-1">Bio</label>
                <textarea className={inputCls + " min-h-[100px] resize-y"} placeholder="Tell others about yourself..."
                  value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-surface-700 mb-1">Areas of Interest</label>
                <input type="text" className={inputCls} placeholder="e.g. Machine Learning, Web Dev, DevOps (comma separated)"
                  value={form.areasOfInterest} onChange={e => setForm(f => ({ ...f, areasOfInterest: e.target.value }))} />
                <p className="text-xs text-surface-400 mt-1">Separate multiple interests with commas</p>
              </div>
            </div>
          ) : (
            <>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {profile.links.map((link, i) => (
                      <a key={i} href={link.url} target="_blank" rel="noopener noreferrer"
                        ...
                        {link.platform === 'github' ? '🔗 GitHub' : link.platform === 'linkedin' ? '🔗 LinkedIn' : link.platform === 'twitter' ? '🔗 Twitter' : link.platform === 'portfolio' ? '🔗 Portfolio' : '🔗 Link'}
                      </a>
                    ))}
                  </div>
                </div>
